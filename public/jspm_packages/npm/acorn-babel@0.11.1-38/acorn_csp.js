...
(function(process) {
  (function(root, mod) {
    ...
    var _jsxTagStart = {type: "jsxTagStart"},
        _jsxTagEnd = {type: "jsxTagEnd"};
    exports.tokTypes = {
      bracketL: _bracketL,
      bracketR: _bracketR,
      braceL: _braceL,
      ...
      regexp: _regexp,
      string: _string,
      paamayimNekudotayim: _paamayimNekudotayim,
      exponent: _exponent,
      hash: _hash,
      arrow: _arrow,
      template: _template,
    ...
    function finishToken(type, val) {
      ...
      if (type === _parenR || type === _braceR) {
        ...
        tokExprAllowed = true;
      } else if (type == _incDec) {} else if (type.keyword && prevType == _dot) {
        tokExprAllowed = false;
        ...
        tokContext.push(j_expr);
        tokContext.push(j_oTag);
        tokExprAllowed = false;
    ...
    function skipBlockComment() {
      ...
      if (options.onComment)
        options.onComment(true, input.slice(start + 2, end), start, tokPos, startLoc, options.locations && curPosition());
    }
    function skipLineComment(startSkip) {
      ...
      if (options.onComment)
        options.onComment(false, input.slice(start + startSkip, tokPos), start, tokPos, startLoc, options.locations && curPosition());
    }
    function skipSpace() {
    ...
    function readToken_pipe_amp(code) {
      var next = input.charCodeAt(tokPos + 1);
      if (next === code) {
        if (options.playground && input.charCodeAt(tokPos + 2) === 61)
    ...
    function readJSXToken() {
      ...
      for (; ; ) {
        if (tokPos >= inputLen)
          raise(tokStart, "Unterminated JSX contents");
        var ch = input.charCodeAt(tokPos);
        switch (ch) {
          case 123:
          case 60:
    ...
    function parseForStatement(node) {
      ...
      if (tokType === _var || tokType === _let) {
        ...
        if ((tokType === _in || (options.ecmaVersion >= 6 && isContextual("of"))) && init.declarations.length === 1 && !(isLet && init.declarations[0].init))
    ...
    function parseMaybeAssign(noIn, refShorthandDefaultPos, afterLeftParse) {
      ...
      if (afterLeftParse)
        left = afterLeftParse(left, start);
      if (tokType.isAssign) {
        var node = startNodeAt(start);
      ...
      if (eat(_question)) {
        ...
        if (options.playground && eat(_eq)) {
          ...
          if (left.type !== "MemberExpression")
            raise(left.start, "You can only use member expressions in memoization assignment");
          node.right = parseMaybeAssign(noIn);
          node.operator = "?=";
          return finishNode(node, "AssignmentExpression");
        }
    ...
    function parseParenAndDistinguishExpression(start, isAsync) {
      ...
      if (options.ecmaVersion >= 6) {
        ...
        var innerStart = storeCurrentPos(),
            exprList = [],
            first = true;
        var refShorthandDefaultPos = {start: 0},
            spreadStart,
    ...
    function parseObj(isPattern, refShorthandDefaultPos) {
      ...
      while (!eat(_braceR)) {
        ...
        } else if (options.ecmaVersion >= 5 && !prop.computed && prop.key.type === "Identifier" && (prop.key.name === "get" || prop.key.name === "set" || (options.playground && prop.key.name === "memo")) && (tokType != _comma && tokType != _braceR)) {
    ...
    function parseClass(node, isStatement) {
      ...
      while (!eat(_braceR)) {
        ...
        if (tokType !== _parenL && !method.computed && method.key.type === "Identifier" && method.key.name === "static") {
        ...
        if (tokType !== _parenL && !method.computed && method.key.type === "Identifier" && method.key.name === "async") {
          isAsync = true;
          parsePropertyName(method);
        }
        if (tokType !== _parenL && !method.computed && method.key.type === "Identifier" && (method.key.name === "get" || method.key.name === "set") || (options.playground && method.key.name === "memo")) {
          if (isGenerator || isAsync)
            unexpected();
    ...
    function parseJSXElementAt(start) {
      ...
      if (!openingElement.selfClosing) {
        ...
        if (getQualifiedJSXName(closingElement.name) !== getQualifiedJSXName(openingElement.name)) {
          raise(closingElement.start, "Expected corresponding JSX closing tag for <" + getQualifiedJSXName(openingElement.name) + ">");
        }
      }
      ...
      switch (tokType) {
        ...
        case _parenL:
          ...
          if (isGroupedType) {
            ...
            if (eat(_arrow)) {
              raise(node, 'Unexpected token =>. It looks like ' + 'you are trying to write a function type, but you ended up ' + 'writing a grouped type followed by an =>, which is a syntax ' + 'error. Remember, function type parameters are named so function ' + 'types look like (name1: type1, name2: type2) => returnType. You ' + 'probably wrote (type1) => returnType');
            }
            return type;
          }
          tmp = parseFunctionTypeParams();
