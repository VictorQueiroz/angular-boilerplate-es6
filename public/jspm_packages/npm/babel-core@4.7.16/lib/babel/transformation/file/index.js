...
var File = (function() {
  ...
  File.options = require("./options.json!systemjs-json");
  File.prototype.normalizeOptions = function normalizeOptions(opts) {
    ...
    for (var key in opts) {
      ...
      if (!option)
        this.log.error("Unknown option: " + key, ReferenceError);
    }
    for (var key in File.options) {
      var option = File.options[key];
      var val = opts[key];
      if (val == null)
        val = option["default"] || null;
    ...
    defaults(opts, {moduleRoot: opts.sourceRoot});
    defaults(opts, {sourceRoot: opts.moduleRoot});
    defaults(opts, {filenameRelative: opts.filename});
    defaults(opts, {
  ...
  File.prototype.addImport = function addImport(source, name, noDefault) {
    ...
    if (!id) {
      ...
      if (noDefault)
        this.dynamicImportedNoDefault.push(declar);
  ...
  File.prototype.setAst = function setAst(ast) {
    ...
    this.ast = ast;
    this.path.traverse({enter: function enter(node, parent, scope) {
        if (this.isScope()) {
  ...
  File.prototype.checkNode = (function(_checkNode) {
    ...
    scope.traverse(node, checkTransformerVisitor, {stack: stack});
  });
