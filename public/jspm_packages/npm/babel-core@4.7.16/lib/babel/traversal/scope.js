...
var functionVariableVisitor = {enter: function enter(node, parent, scope, state) {
    if (t.isFor(node)) {
      each(t.FOR_INIT_KEYS, function(key) {
        var declar = node[key];
        if (t.isVar(declar))
          state.scope.registerBinding("var", declar);
      });
    }
...
var programReferenceVisitor = {enter: function enter(node, parent, scope, state) {
...
var blockVariableVisitor = {enter: function enter(node, parent, scope, state) {
...
var Scope = (function() {
  ...
  Scope.prototype.hasBinding = function hasBinding(name) {
    ...
    if (this.hasOwnBinding(name))
      return true;
    if (this.parentHasBinding(name))
      return true;
    if (includes(Scope.globals, name))
      return true;
    if (includes(Scope.contextVariables, name))
      return true;
    return false;
  };
